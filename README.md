...
- `Blinkit_SQL_Analysis.md` — Converted and annotated project notes (from the original .docx).  
...
Question:
Find the most frequently ordered products by showing each product’s name and the total quantity ordered. Sort the results in descending order of total quantity.
...
The total sales amount (price × quantity)
...
Under 25 → “Gənc” (Young)
Between 25 and 50 → “Yetkin” (Adult)
Over 50 → “Yaşlı” (Senior)
...
Greater than or equal to 15% → “Effektiv” (Effective)
Between 5% and 14% → “Orta” (Medium)
Less than 5% → “Zəif” (Weak)
...
Above 15% → “Yüksək Endirim” (High Discount)
Between 10% and 15% → “Orta Endirim” (Medium Discount)
Below 10% → “Aşağı Endirim” (Low Discount)
...
SELECT 
  ...
  CASE 
    WHEN AVG(coupon_discount) > 15 THEN 'Yüksək Endirim'
